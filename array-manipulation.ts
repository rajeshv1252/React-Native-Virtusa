type Customer = {
  ...
  const mergedCustomer: Customer = { ...customerA, ...customerB };
  
  console.log("Merged Customer:", mergedCustomer);

  //3. Group Customers by City

  function groupByCity(customers: Customer[]): Record<string, Customer[]> {
  ...
  function groupByAgeRange(customers: Customer[]): Record<string, Customer[]> {
    const ranges: Record<string, Customer[]> = {
      "25-35": [],
      "36-40": [],
      "40+": [],
    };
